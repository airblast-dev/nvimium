...
impl Function {
    pub(crate) fn from_fn<F: 'static + Send + Sync + Fn(A) -> R + Unpin, A: FromLua, R>(
        f: F,
    ) -> Self {
    ...
    pub fn wrap<F: 'static + Send + Sync + Fn(A) -> R + Unpin, A: FromLua, R: IntoLua>(
        f: F,
    ) -> Self {
